...
interface FormErrorProps {
  error?: string | { fieldErrors?: ZodFormattedError<any>; message?: string };
  className?: string;
}
...
export function FormError({ error, className = '' }: FormErrorProps) {
  ...
  if (typeof error === 'object' && 'fieldErrors' in error && error.fieldErrors) {
    const fieldErrors = Object.entries(error.fieldErrors)
      .filter(([_, errs]) => Array.isArray(errs) && errs.length > 0)
